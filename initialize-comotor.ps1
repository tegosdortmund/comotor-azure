...
if ($SSMS -eq 'Yes') {
  ...
  & $sqlSetupURL -verb=sync /QUIET="True" /ACTION=Install /FEATURES=SSMS /IACCEPTSQLSERVERLICENSETERMS /MEDIALAYOUT=Core
}
...
[Environment]::NewLine
Write-Output '##### Configure NAV service to use the new database #####'
...
[Environment]::NewLine
Write-Output '##### Start NAV service #####'
...
#restart NAV service 
[Environment]::NewLine
Write-Output '##### Restart NAV service #####'
